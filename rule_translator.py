...
class RuleTranslator(nn.Module):
    ...
    def _get_init_state(self, src_seq, src_mask, indeg=None, outdeg=None, dist=None, shortest_path=None, attn_bias=None,
                        link=None, length=None):
        enc_output = None
        enc_output = self.model.encoder(src_seq, indeg, outdeg, dist, attn_bias, shortest_path, link, length)
    ...
    def forward(self, src_seq, trg=None, tailIndexs=None, link=None, padding=300, mode='train', length=None, indeg=None,
                outdeg=None, dist=None, shortest_path=None, attn_bias=None):
        ...
        src_mask = get_pad_mask(src_seq.squeeze(-1), self.src_pad_idx)
        enc_output, gen_seq, scores, dec_output = self._get_init_state(src_seq, src_mask, indeg=indeg, outdeg=outdeg,
                                                                       dist=dist, shortest_path=shortest_path,
                                                                       attn_bias=attn_bias, link=link)
        ...
        for batch in range(batch_size):
            ...
            for step in range(num_step):
                ...
                for rels, pths, wei in paths[step]:
                    ...
                    for rr in torch.nonzero(relation_attention > max(self.the_rel,
                                                                     self.the_rel_min / rel_att_max)):  # relations which exceed threshold
                        rr = rr.item()
                        if rr == self.n_trg_vocab - 1:  # <slf>
                            paths[step + 1].append(
                                (rels + [rr], pths + [pths[-1]], wei * relation_attention[rr].item()))
                        elif rr in self.graph[pths[-1]].keys():
                            for tail in self.graph[pths[-1]][rr]:
                                paths[step + 1].append(
                                    (rels + [rr], pths + [tail], wei * relation_attention[rr].item()))
